...
export async function onRequestGet({ request, env }) {
  ...
  const { results: byService } = await env.DB.prepare(
    `SELECT s.name AS service_name, COUNT(*) AS bookings, COALESCE(SUM(b.price_total), 0) AS revenue
     ...
     WHERE b.status != 'cancelled' GROUP BY b.service_key ORDER BY revenue DESC`
  ).all();

  const { results: todaysBookings } = await env.DB.prepare(
    `SELECT b.*, s.name AS service_name FROM bookings b JOIN services s ON s.key = b.service_key
     WHERE b.date = ? AND b.status != 'cancelled' ORDER BY b.start_time ASC`
  ).bind(today).all();
